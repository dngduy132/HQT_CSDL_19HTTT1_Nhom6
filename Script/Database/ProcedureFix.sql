--Phân hệ khách hàng 
--Chức năng xem danh sách doanh nghiệp 
CREATE OR ALTER PROCEDURE KH_XEM_TATCA_DN_FIX
AS 
	SELECT MADN, TEN_DN,THANH_PHO,QUAN,DIA_CHI
	FROM DOANH_NGHIEP
GO

--Chức năng xem danh sách sản phẩm của doanh nghiệp
CREATE OR ALTER PROCEDURE KH_XEM_SP_THEO_MADN_FIX
( 
	@MADN CHAR(10)
)
AS 
	BEGIN TRAN
		SELECT SP.MA_SP,SP.TEN_SP,SP.GIA,SP.SO_LUONG,SP.MO_TA
		FROM  DOANH_NGHIEP  DN JOIN CHI_NHANH CN
			  ON (DN.MADN=CN.MADN) 
			  JOIN CHINHANH_SANPHAM CN_SP 
			  ON (CN.MA_CN=CN_SP.MA_CN)
			  JOIN SAN_PHAM SP 
			  ON (CN_SP.MA_SP=SP.MA_SP)	
		WHERE DN.MADN=@MADN
	COMMIT TRAN
GO

--chức năng xem tên sản phẩm theo tên doanh nghiệp
CREATE OR ALTER PROCEDURE KH_XEM_TEN_SP_THEO_MADN_FIX
( 
	@TENDN NVARCHAR(50)
)
AS 
	BEGIN TRAN
		SELECT SP.TEN_SP
		FROM  DOANH_NGHIEP  DN  JOIN CHI_NHANH CN 
			  ON (DN.MADN=CN.MADN) 
			  JOIN CHINHANH_SANPHAM CN_SP 
			  ON (CN.MA_CN=CN_SP.MA_CN)
			  JOIN SAN_PHAM SP 
			  ON (CN_SP.MA_SP=SP.MA_SP)	
		WHERE DN.TEN_DN=@TENDN
	COMMIT TRAN
GO

--Chức năng tìm sản phẩm theo tên doanh nghiệp và tên sản phẩm 
CREATE OR ALTER PROCEDURE KH_TIM_SP_THEO_TENDN_TENSP_FIX
(
	@TENDN NVARCHAR(50),
	@TENSP NVARCHAR(50)
)
AS 
	BEGIN TRAN
	SET TRAN ISOLATION LEVEL REPEATABLE READ
		SELECT COUNT(*)
		FROM  DOANH_NGHIEP  DN JOIN CHI_NHANH  CN
			  ON (DN.MADN=CN.MADN) 
			  JOIN CHINHANH_SANPHAM CN_SP
			  ON (CN.MA_CN=CN_SP.MA_CN)
			  JOIN SAN_PHAM SP
			  ON (CN_SP.MA_SP=SP.MA_SP)
		WHERE DN.TEN_DN = @TENDN 
			  AND SP.TEN_SP = @TENSP
	
		WAITFOR DELAY '00:00:01'

		SELECT SP.MA_SP,SP.TEN_SP,SP.GIA,SP.SO_LUONG,SP.MO_TA
		FROM  DOANH_NGHIEP  DN JOIN CHI_NHANH  CN
			  ON (DN.MADN=CN.MADN) 
			  JOIN CHINHANH_SANPHAM CN_SP
			  ON (CN.MA_CN=CN_SP.MA_CN)
			  JOIN SAN_PHAM SP
			  ON (CN_SP.MA_SP=SP.MA_SP)
		WHERE DN.TEN_DN = @TENDN 
			  AND SP.TEN_SP = @TENSP
	COMMIT TRAN
GO


 --Chức năng tìm kiếm doanh nghiệp theo tên doanh nghiệp và tên quận 
 CREATE OR ALTER PROCEDURE TIMDN_THEO_TEN_QUAN_FIX
 (
	@TEN NVARCHAR(50),
	@QUAN NVARCHAR(50)
 )
 AS 
	BEGIN TRAN
	SET TRAN ISOLATION LEVEL REPEATABLE READ
		SELECT COUNT(*)
		FROM DOANH_NGHIEP DN
		WHERE DN.TEN_DN LIKE  @TEN 
			  AND DN.QUAN LIKE @QUAN

		WAITFOR DELAY '00:00:01'

		SELECT * 
		FROM DOANH_NGHIEP DN
		WHERE DN.TEN_DN LIKE  @TEN 
			  AND DN.QUAN LIKE @QUAN
	COMMIT TRAN
 GO

--Chức năng thống kê sản phẩm theo giá
CREATE OR ALTER PROCEDURE KH_THONGKE_SP_GIA_FIX
(
	@MADN CHAR(10),
	@GIA INT 
)
AS 
	BEGIN TRAN
	SET TRANSACTION ISOLATION LEVEL SERIALIZABLE
		--Thống kê số lượng sản phẩm thỏa điều kiện 
		SELECT COUNT(SP.MA_SP)
		FROM  DOANH_NGHIEP  DN JOIN CHI_NHANH  CN
			  ON (DN.MADN=CN.MADN) 
			  JOIN CHINHANH_SANPHAM CN_SP
			  ON (CN.MA_CN=CN_SP.MA_CN)
			  JOIN SAN_PHAM SP
			  ON (CN_SP.MA_SP=SP.MA_SP)
		WHERE DN.MADN = @MADN AND SP.GIA >= @GIA

		WAITFOR DELAY '00:00:01'

		--Hiển thị thông tin sản phẩm thỏa điều kiện 
		SELECT SP.MA_SP,SP.TEN_SP, SP.MO_TA, SP.GIA, SP.SO_LUONG
		FROM  DOANH_NGHIEP  DN JOIN CHI_NHANH  CN
			  ON (DN.MADN=CN.MADN) 
			  JOIN CHINHANH_SANPHAM CN_SP
			  ON (CN.MA_CN=CN_SP.MA_CN)
			  JOIN SAN_PHAM SP
			  ON (CN_SP.MA_SP=SP.MA_SP)
		WHERE DN.MADN = @MADN AND SP.GIA >= @GIA
		ORDER BY SP.GIA DESC
	COMMIT TRAN
GO


-- Chức năng thống kê sản phẩm theo số lượng 
CREATE OR ALTER PROCEDURE KH_THONGKE_SP_SLUONG_FIX
(
	@MADN CHAR(50),
	@SLUONG INT 
)
AS 
	BEGIN TRANSACTION
	SET TRANSACTION ISOLATION LEVEL SERIALIZABLE
		--Thống kê số lượng sản phẩm thỏa điều kiện 
		SELECT COUNT(SP.MA_SP)
		FROM  DOANH_NGHIEP  DN JOIN CHI_NHANH  CN
			  ON (DN.MADN=CN.MADN) 
			  JOIN CHINHANH_SANPHAM CN_SP
			  ON (CN.MA_CN=CN_SP.MA_CN)
			  JOIN SAN_PHAM SP
			  ON (CN_SP.MA_SP=SP.MA_SP)
		WHERE DN.MADN=@MADN AND SP.SO_LUONG = @SLUONG

		WAITFOR DELAY '00:00:01'

		--Hiển thị thông tin sản phẩm thỏa điều kiện 
		SELECT SP.MA_SP,SP.TEN_SP, SP.MO_TA, SP.SO_LUONG
		FROM  DOANH_NGHIEP  DN JOIN CHI_NHANH  CN
			  ON (DN.MADN=CN.MADN) 
			  JOIN CHINHANH_SANPHAM CN_SP
			  ON (CN.MA_CN=CN_SP.MA_CN)
			  JOIN SAN_PHAM SP
			  ON (CN_SP.MA_SP=SP.MA_SP)
		WHERE DN.MADN=@MADN AND SP.SO_LUONG = @SLUONG
		ORDER BY SP.SO_LUONG DESC
	COMMIT TRANSACTION
GO

 --Chức năng mua hàng
CREATE OR ALTER PROCEDURE KH_MUA_HANG_FIX
( 
	@MADON CHAR(10),
	@TENKV NVARCHAR(50),
	@MADN CHAR(10),
	@MAKH CHAR(10),
	@HINHTHUCTHANHTOAN NVARCHAR(50),
	@SOLUONG INT,
	@MASP CHAR(10)
)
AS 
	SET TRANSACTION ISOLATION LEVEL REPEATABLE READ
	BEGIN TRAN
		DECLARE @MAKV CHAR(10)
		SET @MAKV = (SELECT MA_KV FROM KHU_VUC WHERE TEN_KHU_VUC = @TENKV)
			--Thêm mới một đơn đặt hàng  
			INSERT INTO	dbo.DON_DAT_HANG 
			(
				MA_DON,
				MA_KV,
				MADN,
				MA_TX,
				MA_KH,
				NGAY_LAP,
				TINH_TRANG,
				TONG_TIEN_,
				PHI_SAN_PHAM,
				PHI_VAN_CHUYEN,
				HINH_THUC_THANH_TOAN
			)
			VALUES
			(   @MADON,   -- MA_DON - char(10)
				@MAKV,   -- MA_KV - char(10)
				@MADN,   -- MADN - char(10)
				NULL, -- MA_TX - char(10)
				@MAKH,   -- MA_KH - char(10)
				GETDATE(), -- NGAY_LAP - date
				N'Chờ nhận đơn', -- TINH_TRANG - nvarchar(50)
				NULL, -- TONG_TIEN_ - bigint
				NULL, -- PHI_SAN_PHAM - int
				25000, -- PHI_VAN_CHUYEN - int
				@HINHTHUCTHANHTOAN  -- HINH_THUC_THANH_TOAN - nvarchar(50)
			)

			--Thêm mới chi tiết đơn hàng
			DECLARE @GIA INT
			SET @GIA = (SELECT GIA FROM dbo.SAN_PHAM WHERE MA_SP = @MASP)
			INSERT INTO dbo.CT_DON_HANG 
			(
				MA_SP,
				MA_DON,
				SO_LUONG,
				GIA_BAN,
				THANH_TIEN_
			)
			VALUES
			(   @MASP,   -- MA_SP - char(10)
				@MADON,   -- MA_DON - char(10)
				@SOLUONG, -- SO_LUONG - int
				@GIA, -- GIA_BAN - int
				@SOLUONG * @GIA  -- THANH_TIEN_ - int
			)
			DECLARE @SL INT
			SET @SL = (SELECT SO_LUONG FROM dbo.SAN_PHAM WITH(UPDLOCK) WHERE MA_SP = @MASP)
			IF @SL > 0
			BEGIN	
				WAITFOR DELAY '00:00:02'
				UPDATE dbo.SAN_PHAM SET SO_LUONG = @SL - @SOLUONG WHERE MA_SP = @MASP
			END 
			ELSE 
			BEGIN 
				RAISERROR(N'Hết hàng',15,1)
				ROLLBACK
			END 
	COMMIT TRAN
GO
EXEC KH_MUA_HANG_FIX 'HD10122027', N'Huyện Nhà Bè', N'DN71715083', N'KH30426466', N'Chuyển khoản', 2, N'SP29046938' 
--Chức năng mua hàng có deadlock
CREATE OR ALTER PROCEDURE KH_MUA_HANG_DEADLOCK_FIX
( 
	@MADON CHAR(10),
	@TENKV NVARCHAR(50),
	@MADN CHAR(10),
	@MAKH CHAR(10),
	@HINHTHUCTHANHTOAN NVARCHAR(50),
	@SOLUONG INT,
	@MASP CHAR(10)
)
AS 
	BEGIN TRAN
		DECLARE @MAKV CHAR(10)
		SET @MAKV = (SELECT MA_KV FROM KHU_VUC WHERE TEN_KHU_VUC = @TENKV)
			--Thêm mới một đơn đặt hàng  
			INSERT INTO	dbo.DON_DAT_HANG
			(
				MA_DON,
				MA_KV,
				MADN,
				MA_TX,
				MA_KH,
				NGAY_LAP,
				TINH_TRANG,
				TONG_TIEN_,
				PHI_SAN_PHAM,
				PHI_VAN_CHUYEN,
				HINH_THUC_THANH_TOAN
			)
			VALUES
			(   @MADON,   -- MA_DON - char(10)
				@MAKV,   -- MA_KV - char(10)
				@MADN,   -- MADN - char(10)
				NULL, -- MA_TX - char(10)
				@MAKH,   -- MA_KH - char(10)
				GETDATE(), -- NGAY_LAP - date
				N'Chờ nhận đơn', -- TINH_TRANG - nvarchar(50)
				NULL, -- TONG_TIEN_ - bigint
				NULL, -- PHI_SAN_PHAM - int
				25000, -- PHI_VAN_CHUYEN - int
				@HINHTHUCTHANHTOAN  -- HINH_THUC_THANH_TOAN - nvarchar(50)
			)

			--Thêm mới chi tiết đơn hàng
			DECLARE @GIA INT
			SET @GIA = (SELECT GIA FROM dbo.SAN_PHAM WHERE MA_SP = @MASP)
			INSERT INTO dbo.CT_DON_HANG
			(
				MA_SP,
				MA_DON,
				SO_LUONG,
				GIA_BAN,
				THANH_TIEN_
			)
			VALUES
			(   @MASP,   -- MA_SP - char(10)
				@MADON,   -- MA_DON - char(10)
				@SOLUONG, -- SO_LUONG - int
				@GIA, -- GIA_BAN - int
				@SOLUONG * @GIA  -- THANH_TIEN_ - int
			)
			DECLARE @SL INT
			SET @SL = (SELECT SO_LUONG FROM dbo.SAN_PHAM WITH(UPDLOCK) WHERE MA_SP = @MASP)
			IF @SL > 0
			BEGIN	
				WAITFOR DELAY '00:00:02'
				UPDATE dbo.SAN_PHAM SET SO_LUONG = @SL - @SOLUONG WHERE MA_SP = @MASP
			END 
			ELSE 
			BEGIN 
				RAISERROR(N'Hết hàng',15,1)
				ROLLBACK
			END 
	COMMIT TRAN
GO

--Phân hệ tài xế 
--Chức năng xem danh sách đơn đặt hàng
 CREATE OR ALTER PROCEDURE TX_XEM_DH_FIX
 (
	@MATX CHAR(10)
 )
 AS 
	BEGIN TRAN
		DECLARE @KHUVUC CHAR(10)
		SET @KHUVUC = (SELECT MA_KV FROM dbo.TAI_XE  WHERE MA_TX = @MATX)
		SELECT * FROM DON_DAT_HANG
		WHERE MA_TX IS NULL AND MA_KV = @KHUVUC
	COMMIT TRAN
 GO

 --Chức năng tài xế xem mã hóa đơn đang cần giao
 CREATE OR ALTER PROCEDURE TX_XEM_MAHD_FIX
 (
	@MATX CHAR(10)
 )
 AS 
	 BEGIN TRAN
		DECLARE @KHUVUC CHAR(10)
		SET @KHUVUC = (SELECT MA_KV FROM dbo.TAI_XE WHERE MA_TX = @MATX)
		SELECT MA_DON FROM DON_DAT_HANG
		WHERE MA_TX IS NULL AND MA_KV = @KHUVUC
	COMMIT TRAN
 GO

 --Chức năng nhận đơn hàng 
 CREATE OR ALTER PROCEDURE TX_NHAN_DH_FIX
 (
	@MATX CHAR(10),
	@MADH CHAR(10)
 )
 AS 
	BEGIN TRANSACTION
		IF(SELECT MA_TX FROM dbo.DON_DAT_HANG WITH(UPDLOCK) WHERE  MA_DON= @MADH)  IS NULL
		BEGIN	
			WAITFOR DELAY '00:00:05'
			UPDATE dbo.DON_DAT_HANG SET MA_TX = @MATX WHERE MA_DON = @MADH
		END 
		ELSE 
		BEGIN 
			RAISERROR(N'ĐƠN HÀNG ĐÃ ĐƯỢC NHẬN',15,1)
			ROLLBACK
		END 
	COMMIT TRAN
GO

--Nhận đơn hàng deadlock
CREATE OR ALTER PROCEDURE TX_NHAN_DH_DEADLOCK_FIX
 (
	@MATX CHAR(10),
	@MADH CHAR(10)
 )
 AS 
	BEGIN TRANSACTION
	SET TRAN ISOLATION LEVEL REPEATABLE READ
		IF(SELECT MA_TX FROM dbo.DON_DAT_HANG WITH(UPDLOCK) WHERE MA_DON= @MADH)  IS NULL
		BEGIN	
			WAITFOR DELAY '00:00:05'
			UPDATE dbo.DON_DAT_HANG SET MA_TX = @MATX WHERE MA_DON = @MADH
		END 
		ELSE 
		BEGIN 
			RAISERROR(N'ĐƠN HÀNG ĐÃ ĐƯỢC NHẬN',15,1)
			ROLLBACK
		END 
	COMMIT TRAN
GO

--Phân hệ doanh nghiệp
-- Chức năng thống kê hóa đơn 
--lỗi--
CREATE OR ALTER PROCEDURE DN_TKE_DON_DAT_HANG_FIX
(
	@MADN CHAR(10)
)
AS 
	SET TRANSACTION ISOLATION LEVEL SERIALIZABLE
	BEGIN TRAN
		-- Đếm số hóa đơn của doanh nghiệp
		SELECT COUNT(MA_DON) 
		--FROM dbo.DOANH_NGHIEP DN JOIN dbo.DON_DAT_HANG HD ON (HD.MADN = DN.MADN)
		--WHERE DN.MADN = @MADN
		FROM DON_DAT_HANG 
		WHERE MADN = @MADN

		WAITFOR DELAY '00:00:03'

		-- Hiển thị thông tin hóa đơn của doanh nghiệp
		SELECT * FROM DON_DAT_HANG WHERE MADN = @MADN
	COMMIT TRAN
GO 

 --Chức năng cập nhật thông tin sản phẩm 
 CREATE OR ALTER PROCEDURE CAPNHAT_TTIN_SP_FIX
 (
	@MASP CHAR(10),
	@TENSP NVARCHAR(50),
	@GIA INT, 
	@SOLUONG INT
 )
 AS  
	BEGIN TRAN
		WAITFOR DELAY '00:00:02'
		UPDATE SAN_PHAM 
		SET TEN_SP = @TENSP, SO_LUONG = @SOLUONG, GIA=@GIA
		WHERE MA_SP=@MASP
		--WAITFOR DELAY '00:00:07'
		--ROLLBACK
	COMMIT TRAN
 GO 

 --Chức năng cập nhật thông tin sản phẩm có Rollback
  CREATE OR ALTER PROCEDURE CAPNHAT_TTIN_SP_ROLLBACK_FIX
 (
	@MASP CHAR(10),
	@TENSP NVARCHAR(50),
	@GIA INT, 
	@SOLUONG INT
 )
 AS  
	BEGIN TRAN 
		--WAITFOR DELAY '00:00:02'
		UPDATE SAN_PHAM 
		SET TEN_SP = @TENSP, SO_LUONG = @SOLUONG, GIA=@GIA
		WHERE MA_SP=@MASP
		WAITFOR DELAY '00:00:07'
		ROLLBACK
 GO

 
 --Chức năng thêm mới sản phẩm 
CREATE OR ALTER PROCEDURE THEM_SP_FIX
(
	@MACN CHAR(10),
	@MASP CHAR(10),
	@TENSP NVARCHAR(50),
	@GIA INT,
	@SOLUONG INT,
	@MOTA NVARCHAR(100)
)
 AS 
	BEGIN TRAN
		INSERT INTO SAN_PHAM VALUES( @MASP,@TENSP,@GIA,@SOLUONG,@MOTA)
		INSERT INTO CHINHANH_SANPHAM VALUES( @MACN, @MASP)
	COMMIT TRAN
 GO

--Phân hệ quản trị 
--Chức năng xem danh sách doanh nghiệp 
 CREATE OR ALTER PROCEDURE AD_XEM_DN_FIX
 AS
	 BEGIN TRAN
		 SELECT * 
		 FROM DOANH_NGHIEP
	 COMMIT TRAN
 GO

 --Chức năng cập nhật thông tin doanh nghiệp
 CREATE OR ALTER PROCEDURE CAPNHAT_DN_FIX
 (
	 @MADN CHAR(10),
	 @QUAN NVARCHAR(50)
 )
 AS 
	 BEGIN TRAN
		 UPDATE DOANH_NGHIEP 
		 SET QUAN=@QUAN
		 WHERE MADN=@MADN
	COMMIT TRAN
GO

--Phân hệ nhân viên 
--Chức năng cập nhật thông tin doanh nghiệp
CREATE OR ALTER PROCEDURE CAPNHAT_DN_NGUOIDAIDIEN_FIX
 (
	 @MADN CHAR(10),
	 @NGUOIDAIDIEN NVARCHAR(50)
 )
 AS 
	 BEGIN TRAN
		 UPDATE DOANH_NGHIEP 
		 SET NGUOI_DAI_DIEN = @NGUOIDAIDIEN
		 WHERE MADN=@MADN
	COMMIT TRAN
GO 

--Chức năng cập nhật thông tin hợp đồng
CREATE OR ALTER PROCEDURE CAPNHAT_HD_NGUOIDAIDIEN_FIX
 (
	 @MADN CHAR(10),
	 @NGUOIDAIDIEN NVARCHAR(50)
 )
 AS 
	 BEGIN TRAN
		 UPDATE dbo.HOP_DONG 
		 SET NGUOI_DAI_DIEN = @NGUOIDAIDIEN
		 WHERE MADN=@MADN
	COMMIT TRAN
GO